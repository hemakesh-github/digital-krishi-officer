...
export default function ExpertDashboard() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            console.error(e)
            setError(e?.response?.data?.detail || 'Failed to load dashboard. Is the backend running?')
        } finally {
            setLoading(false)
        }
    }, [])
    ...
    return (
        <div className="min-h-screen bg-[#f2f6f2] p-4 lg:p-8">
            ...
            <div className="flex flex-wrap items-start justify-between gap-3 mb-7">
                <div>
                    <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900">Expert Dashboard</h1>
                    <p className="text-sm text-gray-500 mt-1">Review, respond &amp; resolve farmer queries</p>
                </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
                <StatCard emoji="⏳" label="Pending" count={pending.length} sub="Awaiting your response" bg="bg-amber-50" />
                <StatCard emoji="✅" label="Answered" count={answered.length} sub="Sessions resolved" bg="bg-green-50" />
            </div>

            {/* Tabs */}
            <div className="flex gap-2 mb-6 bg-gray-100 p-1 rounded-xl overflow-x-auto">
                {[['pending', 'Pending Queries', pending.length, 'amber'], ['answered', 'Answered', answered.length, 'green']].map(([tab, label, count, color]) => (
            ...
            {/* Lists */}
            {activeTab === 'pending'
                ? <PendingList queries={pending} onOpen={handleOpenSession} />
                : <AnsweredList queries={answered} onOpen={handleOpenSession} />
            }
        </div>
    )
}
...
// ── Pending list ──────────────────────────────────────────────────────────
function PendingList({ queries, onOpen }) {
    if (!queries.length) return (
        <div className="bg-white rounded-2xl p-12 text-center border border-gray-200">
            <div className="text-4xl mb-3">✅</div>
            <p className="text-gray-600">No pending queries — all caught up!</p>
        </div>
    )
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-green-600 uppercase tracking-wide">⏳ Pending Queries</span>
                <span className="text-xs text-gray-400">{queries.length} ITEMS</span>
            </div>
                    ...
                    >
                        <div className="flex items-start gap-4">
                            <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-xl shrink-0 ${isEscalated ? 'bg-red-50' : 'bg-amber-50'}`}>
                                {isEscalated ? '🚨' : cropEmoji(cd.crop)}
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                    <span className="font-bold text-gray-900 capitalize">{cd.crop || 'Unknown'}</span>
...
// ── Answered list ─────────────────────────────────────────────────────────
function AnsweredList({ queries, onOpen }) {
    if (!queries.length) return (
        <div className="bg-white rounded-2xl p-12 text-center border border-gray-200">
            <div className="text-4xl mb-3">📭</div>
            <p className="text-gray-600">No answered queries yet.</p>
        </div>
    )
    ...
    return (
        <div className="space-y-3">
            <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-green-600 uppercase tracking-wide">✅ Answered</span>
                <span className="text-xs text-gray-400">{queries.length} ITEMS</span>
            </div>
            <div className="h-px bg-gray-200 mb-4" />
            {queries.map((q, i) => {
                    ...
                    >
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 flex-wrap">
                                    {cd.location && <span className="text-xs px-2 py-1 rounded-full bg-gray-100 text-gray-700">📍 {cd.location}</span>}
                                    <span className="text-xs px-2 py-1 rounded-full bg-green-100 text-green-700">Answered</span>
                                </div>
                                <div className="text-xs text-gray-400 font-mono mt-2">{q.session_id}</div>
                            </div>
                            <div className="flex flex-col items-end gap-2 shrink-0">
